...
class OverrideFlatModCalView extends DefaultModCalView {
	function _displayCalendarMod($time, $startday, $linkString, &$day_name, $monthMustHaveEvent = false, $basedate = false) {
		...
		if ($this->minical_prevmonth) {
			...
			// $linkprevious = '<img border="0" title="' . JText::_("JEV_PREVIOUSMONTH") . '"
			// alt="' . JText::_ ( "JEV_LAST_MONTH" ) . '"
			// class="mod_events_link"
			// src="' . $viewimages . '/mini_arrowleft.gif"
			// onmousedown="callNavigation(\'' . $linkprevious . '\');"
			// ontouchstart="callNavigation(\'' . $linkprevious . '\');" />';
			$linkprevious = '<i class="fas fa-chevron-circle-left"
			title="' . JText::_("JEV_PREVIOUSMONTH") . '"
			onmousedown="callNavigation(\'' . $linkprevious . '\');"
			ontouchstart="callNavigation(\'' . $linkprevious . '\');" />';
		} else {
			$linkprevious = "";
		...
		if ($this->minical_nextmonth) {
			...
			// $linknext = '<img border="0"
			// title="' . JText::_("JEV_NEXT_MONTH") . '"
			// alt="' . JText::_ ( "JEV_NEXT_MONTH" ) . '"
			// class="mod_events_link"
			// src="' . $viewimages . '/mini_arrowright.gif"
			// onmousedown="callNavigation(\'' . $linknext . '\');"
			// ontouchstart="callNavigation(\'' . $linknext . '\');" />';
			$linknext = '<i class="fas fa-chevron-circle-right"
			title="' . JText::_("JEV_PREVIOUSMONTH") . '"
			onmousedown="callNavigation(\'' . $linknext . '\');"
			ontouchstart="callNavigation(\'' . $linknext . '\');" />';
...
<div id="flatcal_minical">
	<table width="' . $width . '" cellspacing="1" cellpadding="0" border="0" align="center" class="flatcal_main_t">
		...
		if ($this->minical_showlink) {
			$content .= '
				<table style="width:100%;" cellspacing="0" cellpadding="2" border="0" class="flatcal_navbar table">
					<tr>
						<td class="link_prev text-left">' . $linkprevious . '</td>
		        <td class="flatcal_month_label text-center">' . $linkcurrent . '</td>
						<td class="link_next text-right">' . $linknext . '</td>
					</tr>
				</table>';
		}
		$content .= '<table style="width:100%; " class="flatcal_weekdays table">';

		$lf = "\n";

		// Days name rows - with blank week no.
		$content .= "<tr class='flatcal_weekdays_row'>\n";
		for($i = 0; $i < 7; $i ++) {
