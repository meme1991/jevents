...
class OverrideFlatModLatestView extends DefaultModLatestView
{
	...
	function displayLatestEvents(){
		...
		$content = "<section class='mod_jevents_latest wrapper'><div class='container'><div class='row'>";

		if(isset($this->eventsByRelDay) && count($this->eventsByRelDay)){

			$content .= $this->modparams->get("modlatest_templatetop") || $this->modparams->get("modlatest_templatebottom") ? $this->modparams->get("modlatest_templatetop") : '<div class="col-12">';
			...
			foreach($this->eventsByRelDay as $relDay => $daysEvents){
				...
				foreach($daysEvents as $dayEvent){
					...
					$dst = "border-left: 10px ".$dayEvent->bgcolor()." solid; border-top: 1px #eee solid;";
					if($firstTime) $eventrow = '<div class="mod_events_latest_first mb-2 px-3" style="'.$dst.'">%s'."</div>\n";
					else $eventrow = '<div class="mod_events_latest mb-2 px-3" style="'.$dst.'">%s'."</div>\n";
